...
fbuilderjQuery(window).bind('pageshow', function (event) {
  if (typeof event.originalEvent['persisted'] != 'undefined' && event.originalEvent['persisted'])
    location.reload()
})
  ...
  ;(function ($) {
    ...
    fbuilderjQuery['fbuilder']['modules']['default'] = {
      ...
      callback: function () {
        ...
        if (window.REMAINDER == undefined) {
          window.REMAINDER = window.remainder = function (a, b) {
            return a % b
          }
        }
        function ROUNDx(operation, num, y) {
          if (y && y != 0) {
            var r = operation(num / y) * y,
              p = new String(y).split('.')
        ...
        if (window.MIN == undefined) {
          window.MIN = window.min = function () {
            var l = []
            for (var i in arguments) var l = l.concat(arguments[i])
        ...
        if (window.DEGREES == undefined) {
          window.DEGREES = window.degrees = function (a) {
            return (a * 180) / PI
          }
        }
    ...
    ;(function (root) {
      ...
      lib.DATEDIFF = function (date_one, date_two, date_format, return_format) {
        var d1 = _getDateObj(date_one, date_format),
          ...
          diff,
          r = { years: -1, months: -1, days: -1, hours: -1, minutes: -1, seconds: -1 }
    ...
    ;(function (root) {
      ...
      function _getForm(_form) {
        if (typeof _form == 'undefined') {
          if ('currentFormId' in fbuilderjQuery.fbuilder)
            _form = fbuilderjQuery.fbuilder.currentFormId
          else return '_1'
        }
        if (/^_\d*$/.test(_form)) return _form
        if (/^\d*$/.test(_form)) return '_' + _form
        return $(typeof _form == 'object' ? _form : '#' + _form)
          .find('[name="cp_calculatedfieldsf_pform_psequence"]')
          .val()
      }
      ...
      lib.activatefield = lib.ACTIVATEFIELD = function (_field, _form) {
        var o = _getForm(_form),
          f = _getField(_field, _form),
          j
        if (f) {
          j = f.jQueryRef()
          if (j.find('[id*="' + f.name + '"]').hasClass('ignore')) {
            ...
            if (!(f.name in $.fbuilder.forms[o].toShow))
              $.fbuilder.forms[o].toShow[f.name] = { ref: {} }
            j.find('[id*="' + f.name + '"]')
              .removeClass('ignore')
              .change()
    ...
    fbuilderjQuery['fbuilder']['modules']['processing'] = {
      ...
      callback: function () {
        fbuilderjQuery['fbuilder']['extend_window'](
          fbuilderjQuery['fbuilder']['modules']['processing']['prefix'],
          CF_FIELDS_MANAGEMENT
        )
      }
    }
    ;(function (root) {
      ...
      lib.cffProxy =
        ...
        lib.CFFPROXY =
          function () {
            if (
              typeof fbuilderjQuery == 'undefined' ||
              !arguments.length ||
            ...
            var form_id =
                typeof $.fbuilder['currentFormId'] != 'undefined'
                  ? $.fbuilder['currentFormId']
    ...
    fbuilderjQuery['fbuilder']['modules']['connector'] = {
      ...
      callback: function () {
        fbuilderjQuery['fbuilder']['extend_window'](
          fbuilderjQuery['fbuilder']['modules']['connector']['prefix'],
          CF_CONNECTOR
        )
      }
    }
        ...
        lib.GETURLPATH =
          function (noslash) {
            return window.location.pathname
              .replace(noslash ? new RegExp('^/', 'g') : '', '')
              .replace(noslash ? new RegExp('/$', 'g') : '', '')
          }
      lib.getURLParameters =
        lib.geturlparameters =
        lib.GETURLPARAMETERS =
          function (url) {
    ...
    fbuilderjQuery['fbuilder']['modules']['url'] = {
      ...
      callback: function () {
        fbuilderjQuery['fbuilder']['extend_window'](
          fbuilderjQuery['fbuilder']['modules']['url']['prefix'],
          CF_URL
        )
      }
    }
    ...
    $.fbuilder['htmlDecode'] = function (value) {
      return /&(?:#x[a-f0-9]+|#[0-9]+|[a-z0-9]+);?/gi.test(value)
        ? $('<div/>').html(value).text()
        : value
    }
    $.fbuilder['escapeSymbol'] = function (value) {
    ...
    $.fbuilder['parseVal'] = function (value, thousand, decimal) {
      ...
      var t = new String(value)
          .replace(
            new RegExp((/^\s*$/.test(thousand) ? ',' : thousand) + '(\\d{1,2})$'),
            decimal + '$1'
          )
    ...
    $.fbuilder['manageHistory'] = function (onload) {
      ...
      if (b) {
        while ((m = b.match(/f(\d+)p(\d+)\|?/))) {
          ...
          flag =
            !(f in $.fbuilder.forms) ||
            t != $.fbuilder['goToPage']({ formIdentifier: f, from: 0, to: t })
        }
      } else {
    ...
    $.fbuilder['goToPage'] = function (config) {
      if (('formIdentifier' in config || 'form' in config) && 'to' in config) {
        var id = config['form']
            ? $('[name="cp_calculatedfieldsf_pform_psequence"]', config['form']).val()
            : config['formIdentifier'],
          ...
          if (items[i]) {
            if (isNotFirstTime) {
              ...
              if (items[i].usedInEquations)
                $.fbuilder['calculator'].enqueueEquation(id, items[i].usedInEquations)
            }
      ...
      if ('formIdentifier' in config) {
        ...
        if (id in $.fbuilder['forms']) {
          var toShow = $.fbuilder['forms'][id]['toShow'],
            ...
            items =
              'fieldIdentifier' in config
                ? [
                    $.fbuilder['forms'][id].getItem(
                      config['fieldIdentifier'].replace(/_[cr]b\d+$/i, '')
                    )
                  ]
                : $.fbuilder['forms'][id].getItems()
          processItems(items)
      ...
      if (typeof cpcff_default != 'undefined') {
        ...
        if (id in cpcff_default) {
          ...
          for (var fieldId in data) {
            ...
            try {
              if ('setVal' in item) item.setVal(data[fieldId], true)
            } catch (err) {}
          }
    ...
    $.fn.fbuilder = function (options) {
      var opt = $.extend({}, { pub: false, identifier: '', title: '' }, options, true)
      opt.messages = $.extend(
        {
          previous: 'Previous',
          next: 'Next',
      ...
      $('#cp_calculatedfieldsf_pform' + fnum).validate({
        ...
        errorPlacement: function (e, element) {
          ...
          e.addClass('message')
            ...
            .appendTo(_uh.length ? _uh : _parent)
        }
      ...
      var items = [],
        ...
        reloadItemsPublic = function () {
          var form_tag = $('#cp_calculatedfieldsf_pform' + opt.identifier),
            ...
            getCaptchaHTML = function () {
              ...
              if (captcha_tag.length) {
                html +=
                  '<div class="captcha">' + captcha_tag.html() + '</div><div class="clearer"></div>'
                captcha_tag.remove()
              }
              return html
            },
            ...
            for (i; i < items.length; i++) {
              ...
              if (items[i].ftype == 'fPageBreak') {
                ...
                if (items[i].predefinedClick) {
                  page_tag
                    .find('#' + items[i].name)
                    .attr({ placeholder: items[i].predefined, value: '' })
                }
                if (items[i].exclude) {
                ...
                if (items[i].userhelpTooltip) {
                  ...
                  if (items[i].userhelp && items[i].userhelp.length) {
                    if (items[i].tooltipIcon)
                      $('<span class="cff-help-icon"></span>')
                        .attr('uh', items[i].userhelp)
                        .appendTo($(uh.children('label')[0] || uh))
          ...
          if (page > 0) {
              ...
              $('.pbreak', fieldlist_tag).each(function (index) {
                ...
                $(this).html(
                  '<fieldset><legend>' +
                    ...
                    '</legend>' +
                    code +
                    '<div class="pbPrevious" tabindex="0">' +
                    opt.messages.previous +
                    '</div><div class="pbNext" tabindex="0">' +
                    opt.messages.next +
                    '</div>' +
                    bSubmit +
            ...
            if (!opt.cached)
              $('.pb' + page, fieldlist_tag).append(getCaptchaHTML() + getSubmitHTML())
          }
          ...
          jQuery(document).on('click', '.cff-help-icon', function (evt) {
            evt.stopPropagation()
            evt.preventDefault()
          })
          $(document).on('click', '#fbuilder .captcha img', function () {
            ...
            e.attr(
              'src',
              e.attr('src').replace(/&\d+$/, '') + '&' + Math.floor(Math.random() * 1000)
            )
          })
                ...
                open: function (evt, ui) {
                  try {
                    if (window.matchMedia('screen and (max-width: 640px)').matches) {
                      var duration =
                        'undefined' != typeof tooltip_duration && /^\d+$/.test(tooltip_duration)
                          ? tooltip_duration
                          : 3000
                      setTimeout(function () {
      ...
      $.extend(fform.prototype, {
        ...
        after_show: function (id) {
          ...
          if (!this.autocomplete)
            form
              .find('input[name*="fieldname"]:not([autocomplete])')
        ...
        ffunct = {
          ...
          getItem: function (name) {
            ...
            var regExp = new RegExp(
              parseInt(name, 10) == name ? 'fieldname' + name + '_' : name + '_',
              i
            )
            for (var i in items) {
          ...
          loadData: function (f) {
            ...
            if (typeof d != 'undefined') {
              ...
              if (d.length == 2) {
                ...
                opt.setCache =
                  !this.cached && typeof d[1]['setCache'] != 'undefined' && d[1]['setCache']
                    ? true
                    : false
                reloadItemsPublic()
              }
        ...
        if ('string' == typeof s && !/^\s*$/.test(s)) {
          ...
          if (!$.isNumeric(s.charAt(0))) {
            $(document)[i]('change depEvent', s, function (evt) {
              if (me['set_' + attr])
                me['set_' + attr](me._getAttr(attr), $(evt.target).hasClass('ignore'))
            })
          }
        }
      },
      getField: function (f) {
        return $.fbuilder['forms'][this.form_identifier].getItem(f)
      },
      jQueryRef: function () {
        return $('.' + this.name)
      },
    ...
    $.extend($.fbuilder.controls['ftext'].prototype, $.fbuilder.controls['ffields'].prototype, {
      ...
      show: function () {
        ...
        return (
          ...
          '</label><div class="dfield"><input aria-label="' +
          $.fbuilder.htmlEncode(this.title) +
          '" id="' +
          this.name +
          '" name="' +
          this.name +
          '"' +
          (this.minlength.length ? ' minlength="' + this.minlength + '"' : '') +
          (this.maxlength.length ? ' maxlength="' + this.maxlength + '"' : '') +
          (this.equalTo.length ? ' equalTo="#' + this.equalTo + this.form_identifier + '"' : '') +
          ' class="field ' +
          this.size +
      ...
      after_show: function () {
        if (this.regExp != '' && typeof $['validator'] != 'undefined') {
          ...
          this.regExp =
            parts === null ? new RegExp(this.regExp) : new RegExp(parts[2], parts[4].toLowerCase())
    ...
    $.extend($.fbuilder.controls['fcurrency'].prototype, $.fbuilder.controls['ffields'].prototype, {
      ...
      getFormattedValue: function (value) {
        ...
        if (!isNaN(v)) {
          ...
          return (
            me.currencySymbol + sign + (me.noCents ? parts[0] : parts.join(cs)) + me.currencyText
          )
        } else {
          return value
        }
      ...
      show: function () {
        ...
        return (
          ...
          '" name="' +
          this.name +
          '" class="field cffcurrency ' +
          this.size +
          (this.required ? ' required' : '') +
          '" type="text" value="' +
          $.fbuilder.htmlEncode(
            this.formatDynamically ? this.getFormattedValue(this.predefined) : this.predefined
          ) +
          '" ' +
          (!/^\s*$/.test(this.min)
            ? 'min="' +
          ...
          if (!('cffcurrency' in $.validator.methods))
            $.validator.addMethod(
              ...
              function (v, el) {
                ...
                r = new RegExp(
                  '^\\s*(' +
                    esc(e.currencySymbol) +
                    ')?\\s*\\-?\\d+(' +
                    esc(e.thousandSeparator) +
                    '\\d{3})*' +
                    (e.noCents ? '' : '(' + e.centSeparator + '\\d+)?') +
                    '\\s*(' +
                    esc(e.currencyText) +
                    ')?\\s*$',
                  'i'
                ...
                return (
                  this.optional(el) ||
                  r.test(v) ||
                  ($.isNumeric(v) && (!e.noCents || v === FLOOR(v)))
                )
              },
      ...
      val: function (raw) {
        ...
        if (e.length) {
          ...
          v = $.trim(v)
          v = v
            .replace(new RegExp($.fbuilder['escapeSymbol'](this.currencySymbol), 'g'), '')
    ...
    $.extend($.fbuilder.controls['fnumber'].prototype, $.fbuilder.controls['ffields'].prototype, {
      ...
      val: function (raw) {
        ...
        if (e.length) {
          var v = $.trim(e.val())
          if (raw)
            return $.isNumeric(v) && this.thousandSeparator != '.'
              ? v
              : $.fbuilder.parseValStr(v, raw)
    ...
    $.extend($.fbuilder.controls['fslider'].prototype, $.fbuilder.controls['ffields'].prototype, {
      ...
      _setFieldValue: function (nochange) {
        ...
        if (me.range) {
          ...
          $('#' + me.name + '_caption').html(
            me.caption.replace(/\{\s*0\s*\}/, vlf).replace(/\{\s*0\s*\}/, vrf)
          )
        } else {
          ...
          $('#' + me.name + '_caption').html(me.caption.replace(/\{\s*0\s*\}/, vf))
        }
      ...
      init: function () {
        ...
        this.predefinedMin = /^\s*$/.test(this.predefinedMin)
          ? this.min
          : this._toNumber(this.predefinedMin)
        this.predefinedMax = /^\s*$/.test(this.predefinedMax)
          ? this.max
          : this._toNumber(this.predefinedMax)
        this._setHndl('min')
        this._setHndl('max')
        this._setHndl('step')
      ...
      show: function () {
        ...
        function typeValuesComponents() {
          function component(c) {
            var min = $.fbuilder.htmlEncode(me.min),
              ...
              predefined = $.fbuilder.htmlEncode(
                c == 'left' ? me.predefinedMin : c == 'right' ? me.predefinedMax : me.predefined
              ),
              timeoutId
            ...
            return (
              ...
              c +
              '-component"><input aria-label="' +
              $.fbuilder.htmlEncode(me.title) +
              '" id="' +
              me.name +
              '_component_' +
              c +
              '" class="large" type="text" value="' +
              predefined +
              '" ' +
          ...
          if (me.typeValues)
            str +=
              '<div class="slider-type-components ' +
              me.size +
              '">' +
        ...
        me.predefined = /^\s*$/.test(me.predefined)
          ? me.min
          : me._toNumber(me._getAttr('predefined'))
        return (
          '<div class="fields ' +
          me.csslayout +
          ' ' +
          ...
          '<div id="' +
          me.name +
          '_caption" class="slider-caption"></div>' +
    ...
    $.extend($.fbuilder.controls['fcolor'].prototype, $.fbuilder.controls['ffields'].prototype, {
      title: 'Untitled',
      ftype: 'fcolor',
      predefined: '',
      predefinedClick: false,
    ...
    $.extend($.fbuilder.controls['femail'].prototype, $.fbuilder.controls['ffields'].prototype, {
      ...
      show: function () {
        ...
        return (
          ...
          '" ' +
          (this.equalTo != ''
            ? 'equalTo="#' + $.fbuilder.htmlEncode(this.equalTo + this.form_identifier) + '"'
            : '') +
          ' class="field email ' +
          this.size +
          (this.required ? ' required' : '') +
    ...
    $.extend($.fbuilder.controls['fdate'].prototype, $.fbuilder.controls['ffields'].prototype, {
      ...
      get_hours: function () {
        ...
        return (
          ...
          '_hours" name="' +
          me.name +
          '_hours" class="hours-component" aria-label="' +
          $.fbuilder.htmlEncode(me.ariaHourLabel) +
          '" ' +
          (me.readonly ? 'DISABLED' : '') +
          '>' +
          str +
          '</select>:'
        )
      ...
      get_minutes: function () {
        ...
        return (
          ...
          '_minutes" name="' +
          me.name +
          '_minutes" class="minutes-component" aria-label="' +
          $.fbuilder.htmlEncode(me.ariaMinuteLabel) +
          '" ' +
          (me.readonly ? 'DISABLED' : '') +
          '>' +
          str +
      ...
      get_ampm: function () {
        ...
        if (this.tformat == 12) {
          return (
            '<select id="' +
            this.name +
            '_ampm" class="ampm-component"  aria-label="' +
            $.fbuilder.htmlEncode(this.ariaAMPMLabel) +
            '" ' +
      ...
      set_DefaultDate: function (init) {
        var me = this,
          ...
          dp = $('#' + me.name + '_date'),
          dd =
            me.defaultDate != '' ? me.defaultDate : me.predefined != '' ? me.predefined : new Date()
        dp.click(function () {
          $(document).click()
          $(this).focus()
        })
      ...
      set_DefaultTime: function () {
        ...
        if (me.showTimepicker) {
          ...
          if ((parts = /(\d{1,2}):(\d{1,2})\s*([ap]m)?/gi.exec(me.defaultTime)) != null) {
            time['hour'] =
              parts[1] * 1 + (parts.length == 4 && /pm/i.test(parts[3]) && parts[1] != 12 ? 12 : 0)
            time['minute'] = parts[2]
          } else {
            var d = new Date()
          ...
          _setValue(
            me.name + '_minutes',
            time['minute'],
            time['hour'] == me.maxHour ? me.maxMinute : 59
          )
          $('#' + me.name + '_ampm' + ' [value="' + (time['hour'] < 12 ? 'am' : 'pm') + '"]').prop(
            'selected',
            true
          )
        }
      ...
      show: function () {
        var me = this,
          ...
          date_tag_class =
            'field date' +
            me.dformat.replace(/[^a-z]/gi, '') +
            ' ' +
            me.size +
            (me.required && me.showDatepicker ? ' required' : '')
        ...
        return (
          ...
          disabled +
          ' />' +
          (me.showTimepicker ? ' ' + me.get_hours() + me.get_minutes() + ' ' + me.get_ampm() : '') +
          '<span class="uh">' +
          me.userhelp +
          '</span></div><div class="clearer"></div></div>'
        )
      ...
      after_show: function () {
        var me = this,
          date_format = 'date' + me.dformat.replace(/[^a-z]/gi, ''),
          validator = function (v, e) {
            try {
              ...
              if (i != null)
                return (
                  this.optional(e) ||
                  (i._validateDate() && new RegExp(o.r).test(v) && i._validateTime())
                )
              return true
            } catch (er) {
              return false
    ...
    $.extend($.fbuilder.controls['ftextarea'].prototype, $.fbuilder.controls['ffields'].prototype, {
      ...
      show: function () {
        ...
        return (
          ...
          '' +
          (this.required ? "<span class='r'>*</span>" : '') +
          '</label><div class="dfield"><textarea aria-label="' +
          $.fbuilder.htmlEncode(this.title) +
          '" ' +
    ...
    $.extend($.fbuilder.controls['fcheck'].prototype, $.fbuilder.controls['ffields'].prototype, {
      ...
      show: function () {
        ...
        for (var i = 0, h = this.choices.length; i < h; i++) {
          ...
          str +=
            ...
            i +
            '"><input aria-label="' +
            $.fbuilder.htmlEncode(this.choices[i]) +
            '" name="' +
            this.name +
            '[]" id="' +
            this.name +
            ...
            '" vt="' +
            $.fbuilder.htmlEncode(this.toSubmit == 'text' ? this.choices[i] : this.choicesVal[i]) +
            '" type="checkbox" ' +
            (this.readonly ? ' onclick="return false;" ' : '') +
        ...
        return (
          ...
          str +
          '<div class="clearer"></div><span class="uh">' +
          this.userhelp +
          '</span></div><div class="clearer"></div></div>'
        )
      },
              ...
              for (var j = 0, k = me.choicesDep[i].length; j < k; j++) {
                ...
                if (isHidden || !checked) {
                  if (typeof toShow[dep] != 'undefined') {
                    ...
                    if ($.isEmptyObject(toShow[dep]['ref'])) delete toShow[dep]
                  }
                  if (typeof toShow[dep] == 'undefined') {
                    $('[id*="' + dep + '"],.' + dep)
                      .closest('.fields')
                  ...
                  delete toHide[dep]
                  if (typeof toShow[dep] == 'undefined') toShow[dep] = { ref: {} }
      ...
      val: function (raw) {
        ...
        if (e.length) {
          e.each(function () {
            var t = m
              ? $.fbuilder.parseVal(this.value)
              : $.fbuilder.parseValStr(raw == 'vt' ? this.getAttribute('vt') : this.value, raw)
      ...
      setChoices: function (choices) {
        if ($.isPlainObject(choices)) {
          ...
          if ('dependencies' in choices && $.isArray(choices.dependencies))
            this.choicesDep = choices.dependencies
          var html = this.show(),
            e = $('.' + this.name),
            c = e.attr('class'),
      ...
      getIndex: function () {
        var i = []
        $('[name*="' + this.name + '"]').each(function (j, v) {
          if (this.checked) {
            i.push(j)
          }
        })
    ...
    $.extend($.fbuilder.controls['fradio'].prototype, $.fbuilder.controls['ffields'].prototype, {
      ...
      show: function () {
        ...
        for (var i = 0; i < l; i++) {
          str +=
            ...
            '" type="radio" ' +
            (this.readonly ? ' onclick="return false;" ' : '') +
            (this.choices[i] + ' - ' + this.choicesVal[i] == this.choiceSelected ? 'checked' : '') +
            '/> <span>' +
            $.fbuilder.htmlDecode(this.choices[i]) +
            '</span></label></div>'
        }
      ...
      after_show: function () {
        ...
        if (me.untickAccepted) {
          $(document)
            .off('click', '[id*="' + n + '_"]')
            .on('click', '[id*="' + n + '_"]', function () {
              var m = this,
                e = $(m)
              $('[id*="' + n + '_"]').each(function () {
                if (m !== this) $(this).data('previous-status', false)
              })
              if (e.data('previous-status')) {
                m.checked = false
                e.change()
              }
    ...
    $.extend($.fbuilder.controls['fdropdown'].prototype, $.fbuilder.controls['ffields'].prototype, {
      ...
      show: function () {
        ...
        return (
          ...
          '" ' +
          (this.multiple == true
            ? ' multiple="multiple" size="' + (this.vChoices ? this.vChoices : 1) + '"'
            : '') +
          '>' +
          str +
      ...
      setVal: function (v, nochange) {
        ...
        for (var i in v) {
          t = new String(v[i]).replace(/(['"])/g, '\\$1')
          $(
            '[id="' + n + '"] OPTION[vt="' + t + '"],[id="' + n + '"] OPTION[value="' + t + '"]'
          ).prop('selected', true)
        }
        if (!nochange) $('[id="' + n + '"]').change()
      },
      ...
      getIndex: function () {
        ...
        if (this.multiple) {
          var i = []
          f.find('option').each(function (j, v) {
            if (this.selected) i.push(j)
          })
          return i
    ...
    $.extend($.fbuilder.controls['ffile'].prototype, $.fbuilder.controls['ffields'].prototype, {
      ...
      show: function () {
        ...
        return (
          ...
          this.name +
          '[]"' +
          (this.accept.length ? ' accept="' + this.accept + '"' : '') +
          (this.upload_size.length ? ' upload_size="' + this.upload_size + '"' : '') +
          ' class="field ' +
          this.size +
          (this.required ? ' required' : '') +
          '" ' +
            ...
            for (var i = 0; i < h; i++) {
              ;(function (i, file) {
                if (me.preview && file.type.match('image.*') && 'FileReader' in window) {
                  ...
                  filesContainer.append($('<span>').text((n ? ', ' : '') + file.name))
                  n++
                }
              })(i, this.files[i])
            }
    ...
    $.extend($.fbuilder.controls['fPhone'].prototype, $.fbuilder.controls['ffields'].prototype, {
      ...
      show: function () {
        ...
        for (var i = 0; i < tmp.length; i++) {
          if ($.trim(tmp[i]) != '') {
            str +=
              '<div class="uh_phone" ><input aria-label="' +
              $.fbuilder.htmlEncode(this.title) +
              '" type="text" id="' +
              this.name +
              '_' +
        ...
        if (l) {
          for (var i = 0; i < l; i++) {
            if (!/^\s*$/.test(p[i])) {
              ...
              $(document).on('change', '[id*="' + p[i] + me.form_identifier + '"]', function () {
                me.update()
              })
            }
          }
    ...
    $.fbuilder.controls['fcontainer'].prototype = {
      ...
      after_show: function () {
        ...
        for (var i = 0, h = this.fields.length; i < h; i++) {
          f = $('[id*="' + this.fields[i] + this.form_identifier + '"]')
            .closest('.fields')
            .detach()
          if (this.columns > 1) {
    ...
    $.extend($.fbuilder.controls['ffieldset'].prototype, $.fbuilder.controls['ffields'].prototype, {
      ...
      show: function () {
        return (
          ...
          ' cff-container-field ' +
          (this.collapsible
            ? 'cff-collapsible' + (this.defaultCollapsed ? ' cff-collapsed' : '')
            : '') +
          '" id="field' +
          this.form_identifier +
          '-' +
          this.index +
          '"><FIELDSET>' +
          (!/^\s*$/.test(this.title) ? '<LEGEND>' + this.title + '</LEGEND>' : '') +
          '<div id="' +
          this.name +
          '"></div></FIELDSET><div class="clearer"></div></div>'
        )
      },
      after_show: function () {
        var me = this
        ...
        if (me.collapsible)
          $('.' + me.name + '>fieldset>legend').bind('click', function () {
            $(this).closest('.cff-collapsible').toggleClass('cff-collapsed')
          })
      },
      showHideDep: function (toShow, toHide, hiddenByContainer) {
    ...
    $.extend($.fbuilder.controls['fMedia'].prototype, $.fbuilder.controls['ffields'].prototype, {
      ...
      _show_audio_video: function (d, isV) {
        ...
        return (
          ...
          s.join(' ') +
          '">' +
          ($.trim(d.sSrcAlt) ? '<source src="' + esc(d.sSrcAlt) + '" />' : '') +
          '<p>' +
          d.sFallback +
          '</p></' +
          t +
    ...
    $.extend($.fbuilder.controls['fhtml'].prototype, $.fbuilder.controls['ffields'].prototype, {
      ...
      show: function () {
        ...
        content = content.replace(
          /\(\s*document\s*\)\.one\(\s*['"]showHideDepEvent['"]/gi,
      ...
      $.fbuilder.controls['ffields'].prototype,
      {
        ...
        show: function () {
          ...
          if (!/^\s*$/.test(me.url)) {
            label =
              '<a href="' +
              $.fbuilder.htmlEncode($.trim(me.url)) +
              '" target="_blank">' +
              label +
              '</a>'
            ...
            dlg +=
              '<div class="cff-dialog hide"><span class="cff-close-dlg"></span><div class="cff-dialog-content">' +
              me.message +
              '</div></div>'
          }
          return (
            ...
            me.index +
            '"><div class="dfield">' +
            '<div class="one_column"><label for="' +
            me.name +
            '"><input aria-label="' +
            $.fbuilder.htmlEncode(me.title) +
            '" name="' +
            ...
            $.fbuilder.htmlEncode(me.value) +
            '" vt="' +
            $.fbuilder.htmlEncode(/^\s*$/.test(me.value) ? me.title : me.value) +
            '" type="checkbox" /> <span>' +
            $.fbuilder.htmlDecode(label) +
            '' +
            (me.required ? '<span class="r">*</span>' : '') +
            '</span></label></div>' +
            dlg +
            '<span class="uh"></span></div><div class="clearer"></div></div>'
          )
        },
        after_show: function () {
          $(document).on('click', '.cff-open-dlg', function () {
            var dlg = $(this).closest('.fields').find('.cff-dialog'),
              w = dlg.data('width'),
              h = dlg.data('height')
            dlg.removeClass('hide')
            if ('undefined' == typeof w)
              w = MIN($(this).closest('form').width(), $(window).width(), dlg.width())
            if ('undefined' == typeof h)
              h = MIN($(this).closest('form').height(), $(window).height(), dlg.height())
            dlg.data('width', w)
            dlg.data('height', h)
            dlg.css({
              width: w + 'px',
          ...
          $(document).on('click', '.cff-close-dlg', function () {
            $(this).closest('.cff-dialog').addClass('hide')
          })
        },
        val: function (raw) {
          raw = raw || false
      ...
      $.fbuilder.controls['ffields'].prototype,
      {
        ...
        show: function () {
          ...
          return (
            ...
            this.predefined +
            '"/>' +
            (!this.hidefield ? '<span class="uh">' + this.userhelp + '</span>' : '') +
            '</div><div class="clearer"></div></div>'
          )
        },
        after_show: function () {
          ...
          var e = $('[id="' + me.name + '"]')
          if (me.readonly == false && me.noEvalIfManual)
          ...
          e.bind('calcualtedfield_changed', { obj: me }, function (evt) {
            if (
              $.fbuilder['calculator'].getDepList(
                evt.data.obj.name,
                { value: evt.data.obj.val(), raw: evt.data.obj.val(true) },
        ...
        val: function (raw) {
          ...
          if (e.length) {
            ...
            v = $.trim(v)
            v = v
              .replace(new RegExp($.fbuilder['escapeSymbol'](this.prefix), 'g'), '')
    ...
    $.fbuilder['calculator'] = (function () {
      ...
      _calculate = function (eq, suffix, __ME__) {
        ...
        var _match,
          field_regexp = new RegExp(
            '(fieldname\\d+' + suffix + ')(_[cr]b\\d+)?(\\|[rnv])?([\\D\\b])',
            'i'
          )
        ...
        while ((_match = field_regexp.exec(eq))) {
          ...
          if (field) {
            if (_match[3] && _match[3] == '|n') {
              ...
              v = field.val(
                _match[3] ? (_match[3] == '|v' ? 'vt' : _match[3] == '|r' ? true : false) : false
              )
      ...
      CalcFieldClss.prototype = {
        ...
        removePending: function (form_identifier) {
          if (form_identifier in this.pendings && this.pendings[form_identifier])
            this.pendings[form_identifier]--
        },
        ...
        getDepList: function (calculated_field, values, dependencies) {
          var list = [],
            list_h = []
        ...
        defaultCalc: function (form, enqueued) {
          var dep = false
          form = $(form)
          if (form.length) {
    ...
    var fnum = '_' + fcount
    while (
      typeof window['cp_calculatedfieldsf_fbuilder_config' + fnum] != 'undefined' ||
      fcount < 10
    ) {
      try {
